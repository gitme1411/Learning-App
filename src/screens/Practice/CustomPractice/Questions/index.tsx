...
const QuestionCustomP = ({route, navigation}: any) => {
  const {itemTest, title} = route.params;
  ...
  const [dataQuestions, setDataQuestions] = useState(itemTest.question_info);
  const token = useSelector((state: RootState) => state.auth.token);
  const isAuthenticated = useSelector(
    (state: RootState) => state.auth.isAuthenticated,
  );
  const time = itemTest.total_question_set * itemTest.time_for_one;
  // const time = 20;
  ...
  const onSubmitData = () => {
    ...
    navigation.navigate('PassingCustomPractice', {
      dataQuestions: dataQuestionsNew,
      itemTest: itemPracticeNew,
      save: true,
  ...
  return (
    <LinearGradientView>
      ...
      />
      <CountdownTimer initialTime={time} onFinish={() => onSubmitData()} />
...
export default QuestionCustomP;
