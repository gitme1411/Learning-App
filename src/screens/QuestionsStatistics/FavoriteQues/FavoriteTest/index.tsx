...
const FavoriteTest = ({route, navigation}: any) => {
  const {dataQues, time, dataFavorite} = route.params;
  const {width} = useWindowDimensions();

  const realm = useRealm();
  const listFavoriteQuestion = useQuery(Favorite);
  const dataWrQuestions = dataQues.map(e => {
    return {
      ...e,
      checked: false,
      user_answers: -1,
  ...
  const renderItemFZ = (item, index) => {
    return (
      <View style={{marginLeft: (fullWidth - 160) / 6}}>
        <TouchableOpacity
          onPress={() => {
            setIndexFZ(index);
            setTxtFZ(item.fz);
          ...
          <Image
            source={index == indexFZ ? Icon.icCheckRButton : Icon.icRButton}
          />
        </TouchableOpacity>
      </View>
    );
          ...
          <View
            ...
            <View style={{flexDirection: 'row'}}>
              <View
                ...
                <FlatList
                  data={dataFSize}
                  renderItem={({item, index}) => renderItemFZ(item, index)}
                  horizontal
                  keyExtractor={(item, index) => index.toString()}
                />
              </View>
