...
import {getHardest} from '../../../services/api/api';
import RenderHtml from 'react-native-render-html';
import {moderateScale} from 'react-native-size-matters';
import {setLanguage} from '../../../store/user';
import TextNormal from '../../../components/Text';

const HardestQuestions = ({route, navigation}: any) => {

  const [dataQuestions, setDataQuestions] = useState([]);
  const idLeng = useSelector((state: RootState) => state.user.language);

  const getQuesHardest = async () => {
    const res = await getHardest();
  ...
  const renderItemAns = (item, itemAns) => {
    return (
      <View style={{}}>
        ...
        <TextNormal
          fontSize={16}
          style={{fontWeight: '400', color: '#6D6B7A', marginLeft: 24}}>
          {item.answer_en}
        </TextNormal>
      </View>
    );
