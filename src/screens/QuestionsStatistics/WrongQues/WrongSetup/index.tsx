...
const WrongSetup = ({route, navigation}: any) => {
  const {dataWrong} = route.params;

  const dataQuestion = dataWrong.map(e => {
    return {
      ...e,
      checked: false,
      user_answers: -1,
  ...
  const goWrongTest = () => {
    ...
    // let timeTest = parseInt(time) * dataQues.length;
    // navigation.navigate('WrongTest', {
    //   dataQues: dataQues,
    //   time: timeTest,
    //   dataWrong: dataWrong,
    // });
